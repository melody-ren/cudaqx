...
def test_tensor_network_from_parity_check_basic():
    ...
    for i, t in enumerate(tn.tensors):
        ...
        np.testing.assert_array_equal(t.data, np.array([[1.0, 1.0], [1.0, -1.0]]))
...
def test_tensor_network_from_single_syndrome_mixed():
    ...
    assert len(tn.tensors) == 3
    for i, t in enumerate(tn.tensors):
        expected = np.array([1.0, -1.0]) if syndrome[i] else np.array([1.0, 1.0])
...
def test_prepare_syndrome_data_batch_shape_and_values_randomized():
    np.random.seed(123)
    data = np.random.choice([False, True], size=(4, 5)) # syndrome length 4, 5 syndromes
...
def test_tensor_network_from_syndrome_batch_tags_and_inds_randomized():
    ...
    batch_size = 5
    n_synd = 4
    detection_events = np.random.choice([False, True], size=(batch_size, n_synd))
    syndrome_inds = [f's{i}' for i in range(n_synd)]
    tags = [f'tag{i}' for i in range(n_synd)]
    tn = tensor_network_from_syndrome_batch(detection_events, syndrome_inds, batch_index="batch", tags=tags)
    ...
    for i, t in enumerate(tn.tensors):
        ...
        for b in range(batch_size):
            expected = np.array([1.0, -1.0]) if detection_events[b, i] else np.array([1.0, 1.0])
...
def test_tensor_network_from_logical_observable():
    ...
    obs_inds = ['o0']
    tn = tensor_network_from_logical_observable(obs, obs_inds, ["l0"], logicals_tags=["OBS_0"])
...
def test_tensor_to_cpu_torch_gpu_to_numpy():
    ...
    t = torch.tensor([[1.0, 2.0], [3.0, 4.0]], dtype=torch.float32, device="cuda")
    out = tensor_to_cpu(t, backend="numpy", dtype="float64")
    assert isinstance(out, np.ndarray)
