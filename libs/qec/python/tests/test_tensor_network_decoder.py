...
def test_decoder_init_and_attributes():
    ...
    decoder = qec.get_decoder("tensor_network_decoder", H, logicals=logicals, noise_model=noise)
...
def test_decoder_decode_batch():
    ...
    assert all(isinstance(r.result, list) and 0.0 <= r.result[0] <= 1.0 for r in res)
...
@pytest.mark.parametrize("init_contractor,change_contractor,init_device,change_device,expect_gpu", [
...
def test_decoder_change_contractor(init_contractor, change_contractor, init_device, change_device, expect_gpu):
    H, logicals, noise = make_simple_code()
    import torch

    if ("cuda" in init_device or expect_gpu or "cuda" in change_device) and not torch.cuda.is_available():
        pytest.skip("No GPUs available, skip GPU contractor test.")

    # Initialize decoder with initial contractor and device
    decoder = qec.get_decoder("tensor_network_decoder", H, logicals=logicals, noise_model=noise, contractor_name=init_contractor, device=init_device)
