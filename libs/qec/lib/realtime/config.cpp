...
struct MappingTraits<cudaq::qec::decoding::config::decoder_config> {
  static void mapping(IO &io,
    ...
    // A decoder model comes from exactly one source: stim_dem_path, the flat
    // matrix keys, or dem_chunks. None of the model keys can be mapRequired;
    // form selection happens below, and matrix/DEM content validation is
    // decided by resolve_decoder_init(), not by the parser.
    io.mapOptional("stim_dem_path", config.stim_dem_path, std::string{});
    // A flat configuration names all five matrix fields on the way out as
    // well as the way in. Emitting only the ones that differ from their
    // defaults would drop a legitimately empty O_sparse (a DEM with no
    // observables) and leave behind a document that no longer parses. A
    // chunk-form or DEM-path configuration takes the optional path so its
    // absent fields stay absent, which is what makes the emitted document
    // re-parse as the same form.
    const bool emitting_flat_form =
        io.outputting() && config.stim_dem_path.empty() &&
    ...
    if (chunk_form) {
      // A competing model source, not a derived field: the phases and the
      // Stim DEM would each claim to be the whole model.
      if (!config.stim_dem_path.empty())
        throw std::runtime_error(
            "stim_dem_path must not be set for decoder " +
            std::to_string(config.id) +
            " together with dem_chunks; supply exactly one model source.");
    ...
    } else if (config.stim_dem_path.empty()) {
      // A flat document spells out its whole DEM, so every one of these fields
      // has to be there. They are mapOptional only because the other model
      // sources derive them; without this check a document that omits one
      // parses into a zero-sized DEM and fails much later, at decoder
      // construction. A DEM-path document skips this branch entirely:
      // resolve_decoder_init() decides what it needs.
      if (!io.outputting()) {
        std::string missing;
        ...
        if (!missing.empty())
          throw std::runtime_error(
              ...
              "O_sparse and D_sparse together; use dem_chunks or "
              "stim_dem_path to describe the same DEM another way instead.");
  ...
  llvm::json::Object defs{
      ...
      {"decoder_config",
       llvm::json::Object{
           ...
           // Exactly one model source: the flat matrix keys, a Stim DEM by
           // path, or dem_chunks. The matrix branch needs the dimensions that
           // make its flat encodings interpretable; the DEM branch derives
           // them and forbids the competing matrix keys; the chunk branch
           // derives them from its phases (the parser rejects the derived
           // fields, which is not expressible here). anyOf rather than oneOf
           // because an expanded chunk configuration -- the flat fields plus
           // the dem_chunks they were derived from -- must still validate;
           // each branch carries its own exclusions instead.
           {"anyOf",
            llvm::json::Array{
                llvm::json::Object{
                    {"required",
